...
export async function GET(request: Request) {
  ...
  if (!FORECAST_V3_ENABLED) return NextResponse.json({ enabled: false, operation_status: "blocked", series: [] });
  try {
    ...
    return NextResponse.json({ enabled: true, operation_status: "success", series, generated_at: new Date().toISOString() });
  } catch (error) {
