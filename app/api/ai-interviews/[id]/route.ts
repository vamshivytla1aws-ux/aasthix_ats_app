...
async function authorize(
  id: number,
  permission:
    "ai_interviews.view" | "ai_interviews.update" | "ai_interviews.cancel",
) {
...
export async function GET(
  ...
  try {
    ...
    const [interview, questions, recruiterQuestions, events] = await Promise.all([
      ...
      query(
        `SELECT * FROM ai_interview_events WHERE interview_id=$1 ORDER BY occurred_at DESC LIMIT 100`,
        [id],
      ),
    ]);
...
export async function PATCH(
  ...
  const decision =
    body?.recruiter_decision == null
      ? null
      : String(body.recruiter_decision).toUpperCase();
  if (
    decision &&
    !["PROCEED", "HOLD", "REJECT", "SCHEDULE_HUMAN_INTERVIEW"].includes(
      decision,
    )
  ) {
    return NextResponse.json(
      { error: "Invalid recruiter decision" },
      { status: 400 },
    );
  }
  const result = await query(
    ...
    [
      ...
      decision,
      body?.recruiter_comments ?? null,
    ],
  );
  await query(
    `INSERT INTO ai_interview_audit_events (interview_id,actor_user_id,event_type,metadata_json) VALUES ($1,$2,'UPDATED',$3::jsonb)`,
...
export async function DELETE(
  ...
  const result = await query(
    `UPDATE ai_interviews SET status='CANCELLED',token_revoked_at=NOW(),updated_at=NOW()
      WHERE id=$1 AND status NOT IN ('COMPLETED','CANCELLED','EXPIRED') RETURNING id,status`,
    [id],
  );
  if (!result.rowCount)
    return NextResponse.json(
      { error: "Interview cannot be cancelled in its current status" },
      { status: 409 },
    );
  await query(
    `INSERT INTO ai_interview_audit_events (interview_id,actor_user_id,event_type) VALUES ($1,$2,'CANCELLED')`,
    [id, auth.access.user_id],
  );
  return NextResponse.json({ interview: result.rows[0] });
}
