...
type Row={id:number;title:string;status:string;candidate_name:string;candidate_email:string;job_title:string;difficulty:string;question_count:number;duration_minutes:number;overall_score:number|null;integrity_risk:string|null;evaluation_status:string;created_at:string};

function badge(status:string){
  if(status==="COMPLETED") return "bg-emerald-50 text-emerald-700 ring-emerald-200";
  if(status==="IN_PROGRESS"||status==="PROCESSING") return "bg-blue-50 text-blue-700 ring-blue-200";
...
export default function AiInterviewList(){
  const [rows,setRows]=useState<Row[]>([]); const [loading,setLoading]=useState(true); const [error,setError]=useState(""); const [canDelete,setCanDelete]=useState(false);
  async function load(){setLoading(true);setError("");try{const r=await fetch("/api/ai-interviews",{cache:"no-store"});const d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to load");setRows(d.interviews||[]);setCanDelete(d.configuration?.can_delete===true);}catch(e){setError(e instanceof Error?e.message:"Failed to load");}finally{setLoading(false);}}
  async function remove(row:Row){if(!window.confirm(`Permanently delete the AI interview for ${row.candidate_name}? This also deletes its report and stored media.`))return;setError("");const r=await fetch(`/api/ai-interviews/${row.id}/delete`,{method:"DELETE"});const d=await r.json();if(!r.ok){setError(d.error||"Interview could not be deleted");return;}setRows(current=>current.filter(item=>item.id!==row.id));}
  useEffect(()=>{void load();},[]);
  return <div className="space-y-5">
    <div className="flex flex-wrap items-center justify-between gap-3">
      <div><div className="flex items-center gap-2"><BrainCircuit className="h-7 w-7 text-blue-600"/><h1 className="text-2xl font-bold text-slate-950">AI Interviews</h1></div><p className="mt-1 text-sm text-slate-600">Create secure browser interviews and review performance separately from integrity signals.</p></div>
      <div className="flex gap-2"><button onClick={()=>void load()} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700"><RefreshCw className="h-4 w-4"/>Refresh</button><Link href="/ai-interviews/create" className="inline-flex items-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-blue-700"><Plus className="h-4 w-4"/>Create AI interview</Link></div>
    </div>
