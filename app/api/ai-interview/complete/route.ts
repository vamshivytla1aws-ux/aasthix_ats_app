...
export async function POST(request: Request) {
  ...
  const elapsedSeconds = interview.started_at
    ? Math.max(
        0,
        Math.floor(
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Complete the current adaptive interview question before submitting",
      },
      { status: 409 },
    );
  }
  ...
  try {
    ...
    const result = await client.query(
      `UPDATE ai_interviews SET status='PROCESSING',completed_at=COALESCE(completed_at,NOW()),evaluation_status='PENDING',transcription_status='PENDING',updated_at=NOW()
        WHERE id=$1 AND status='IN_PROGRESS' RETURNING id,status`,
      [interview.id],
    );
    if (!result.rowCount) {
      ...
      if (["PROCESSING", "COMPLETED"].includes(existing.rows[0]?.status))
        return NextResponse.json({
          submitted: true,
  ...
  const queued = await enqueueAiInterview(Number(interview.id)).catch(
    () => false,
  );
  ...
  setImmediate(() => {
    void processAiInterview(Number(interview.id)).catch((error) =>
      console.error("[ai-interviews] background processing", error),
    );
  });
  return NextResponse.json(
    {
      submitted: true,
      status: "PROCESSING",
      processing_mode: queued ? "QUEUED" : "BACKGROUND",
    },
    { status: 202 },
  );
}
