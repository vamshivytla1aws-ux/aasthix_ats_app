...
export default function AnalyticsPage() {
 ...
 >
 {/* Tab bar */}
 <div className="flex flex-wrap items-center gap-1 rounded-xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-1 shadow-sm">
 {TABS.map((t) => (
 <button
 key={t}
 type="button"
...
function TimeFilter({
 ...
 {open && (
 <div className="absolute right-0 z-30 mt-1 w-56 rounded-xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-2 shadow-lg">
...
"block w-full rounded-lg px-3 py-2 text-left text-xs font-medium transition",
 ...
 <label className="block text-xs text-[var(--ats-text-muted)]">From</label>
 <DatePicker value={customFrom} onChange={setCustomFrom} className="text-xs" aria-label="Analytics start date" />
 <label className="block text-xs text-[var(--ats-text-muted)]">To</label>
...
function OverviewTab({ qs }: { qs: string }) {
 ...
 {/* Funnel chart */}
 <div className="lg:col-span-2 rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Hiring Funnel</h3>
 {funnelTotal === 0 ? (
 <div className="py-12 text-center text-sm text-[var(--ats-text-muted)]">No application data to display</div>
 ) : (
 <>
 ...
 {/* Conversion labels */}
 <div className="mt-3 flex flex-wrap items-center justify-center gap-4 text-xs text-[var(--ats-text-muted)]">
 ...
 {/* Activity summary */}
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Activity Summary</h3>
 <div className="space-y-4">
 <ActivityRow label="Interviews today" value={activity.interviews_today} icon={<Calendar className="h-4 w-4 text-[var(--ats-primary)]" />} />
 ...
 <div className="mt-6 border-t border-[var(--ats-border)] pt-4">
 <div className="text-xs font-medium text-[var(--ats-text-muted)]">Quick stats</div>
 <div className="mt-2 grid grid-cols-2 gap-3">
 <div className="rounded-lg bg-[var(--ats-bg-elevated)] p-2.5">
 <div className="text-lg font-bold text-[var(--ats-text)]">{s.total_jobs}</div>
 <div className="text-xs text-[var(--ats-text-muted)]">Total Jobs</div>
 </div>
 <div className="rounded-lg bg-[var(--ats-bg-elevated)] p-2.5">
 <div className="text-lg font-bold text-[var(--ats-text)]">{s.total_applications}</div>
 <div className="text-xs text-[var(--ats-text-muted)]">Applications</div>
 </div>
 </div>
 </div>
 </div>
 ...
 {trend.length > 1 && (
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Application Trend (30 days)</h3>
 ...
 {insights.length > 0 && (
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-3 text-sm font-semibold text-[var(--ats-text)]">Smart Insights</h3>
...
function RecruitersTab({ qs }: { qs: string }) {
 ...
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Recruiter Leaderboard — Closures</h3>
...
function JobsTab({ qs }: { qs: string }) {
 ...
 cell: (r) => (
 <button type="button" onClick={() => setSelectedJob(r.id)} className="text-left font-medium text-[var(--ats-primary)] hover:underline">
 {r.title}
 </button>
 ),
 sortValue: (r) => r.title,
 ...
 <span className="text-[var(--ats-text-muted)]">A→S</span> {r.dropoffs.applied_to_screening}%{""}
 <span className="text-[var(--ats-text-muted)]">S→I</span> {r.dropoffs.screening_to_interview}%{""}
 <span className="text-[var(--ats-text-muted)]">I→Sel</span> {r.dropoffs.interview_to_selected}%
 </div>
 ),
...
function JobDetailPanel({ job, onClose }: { job: JobRow; onClose: () => void }) {
 ...
 return (
 <div className="rounded-2xl border border-indigo-200 bg-[var(--ats-primary)]\/10/30 p-5 shadow-sm dark:border-indigo-800 /20">
 <div className="flex items-start justify-between">
 <div>
 <h3 className="text-base font-semibold text-[var(--ats-text)]">{job.title}</h3>
 <p className="text-xs text-[var(--ats-text-muted)]">{job.company} — <StatusBadge status={job.status ||"Open"} /></p>
 </div>
 <button type="button" onClick={onClose} className="text-xs text-[var(--ats-text-muted)] hover:text-[var(--ats-text)]">Close</button>
 </div>

 <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-5">
 {stages.map((s) => (
 <div key={s.label} className="rounded-xl bg-[var(--ats-bg-panel)] p-3 shadow-sm">
 <div className="text-2xl font-bold" style={{ color: s.color }}>{s.count}</div>
 <div className="text-xs text-[var(--ats-text-muted)]">{s.label}</div>
 </div>
 ))}
 </div>
...
function PipelineTab() {
 ...
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Candidates per Stage</h3>
 ...
 {/* Avg time in stage */}
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Avg Time in Stage (days)</h3>
 <ResponsiveContainer width="100%" height={260}>
 ...
 {flowData.length > 0 && (
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Stage Conversion Flow</h3>
 <div className="flex flex-wrap items-center justify-center gap-4">
 {flowData.map((f) => (
 <div key={`${f.from}-${f.to}`} className="flex items-center gap-2">
 <div className="rounded-lg bg-[var(--ats-bg-subtle)] px-3 py-2 text-xs font-semibold">{f.from}</div>
 ...
 <div className="rounded-lg bg-[var(--ats-bg-subtle)] px-3 py-2 text-xs font-semibold">{f.to}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 <span className="text-[var(--ats-text)]">{b.reason}</span>
...
function SourcesTab() {
 ...
 {/* Pie chart */}
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Applications by Source</h3>
 {pieData.length === 0 ? (
 <div className="py-12 text-center text-sm text-[var(--ats-text-muted)]">No source data</div>
 ) : (
 <ResponsiveContainer width="100%" height={280}>
 <PieChart>
 ...
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Conversion by Source</h3>
 {sources.length === 0 ? (
 <div className="py-12 text-center text-sm text-[var(--ats-text-muted)]">No data</div>
 ) : (
 <ResponsiveContainer width="100%" height={280}>
 ...
 <div className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
 <h3 className="mb-4 text-sm font-semibold text-[var(--ats-text)]">Candidate Sources</h3>
 <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
 {candidate_sources.map((cs) => (
 <div key={cs.source} className="rounded-xl border border-[var(--ats-border)] bg-[var(--ats-bg-elevated)] p-3">
 <div className="text-lg font-bold text-[var(--ats-text)]">{cs.count}</div>
 <div className="text-xs text-[var(--ats-text-muted)]">{cs.source}</div>
 </div>
 ))}
 </div>
 </div>
...
function SummaryCard({ label, value, icon, color }: { label: string; value: number; icon: React.ReactNode; color: string }) {
 ...
 indigo:"bg-indigo-100 text-[var(--ats-primary)]",
 ...
 return (
 <div className={`rounded-2xl border border-[var(--ats-border)] border-t-4 bg-gradient-to-br p-5 shadow-sm transition hover:shadow-md ${bg[color] ?? bg.indigo}`}>
 <div className="flex items-start justify-between">
 <div>
 <div className="text-xs font-medium text-[var(--ats-text-muted)]">{label}</div>
 <div className="mt-1 text-2xl font-bold tracking-tight text-[var(--ats-text)]">{value.toLocaleString()}</div>
 </div>
...
function ActivityRow({ label, value, icon, warn }: { label: string; value: number; icon: React.ReactNode; warn?: boolean }) {
 ...
 {icon}
 <div className="flex-1 text-sm text-[var(--ats-text)]">{label}</div>
 <div className={`text-lg font-bold ${warn ?"text-red-600" :"text-[var(--ats-text)]"}`}>
 {value}
 </div>
 </div>
 );
...
function InsightRow({ type, message }: { type:"warning" |"success" |"info"; message: string }) {
 ...
 {icons[type]}
 <span className="text-sm text-[var(--ats-text)]">{message}</span>
 </div>
 );
}
...
function DaysOpenBadge({ days }: { days: number }) {
 const color = days > 30 ?"text-red-700 bg-red-50" : days > 14 ?"text-amber-700 bg-amber-50" :"text-[var(--ats-text)] bg-[var(--ats-bg-elevated)]";
 return <span className={`inline-flex rounded-full px-2 py-0.5 text-xs font-semibold ${color}`}>{days}d</span>;
}

function MiniStat({ label, value, warn }: { label: string; value: string; warn?: boolean }) {
 return (
 <div className="rounded-xl bg-[var(--ats-bg-panel)] p-3 shadow-sm">
 <div className={`text-lg font-bold ${warn ?"text-red-600" :"text-[var(--ats-text)]"}`}>{value}</div>
 <div className="text-xs text-[var(--ats-text-muted)]">{label}</div>
 </div>
 );
}
...
function SkeletonCards({ count }: { count: number }) {
 ...
 <div key={i} className="rounded-2xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-5 shadow-sm">
