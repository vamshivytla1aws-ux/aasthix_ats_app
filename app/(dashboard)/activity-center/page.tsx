...
export default function ActivityCenterPage() {
 ...
 <span>
 <span className="font-semibold text-[var(--ats-text)]">{rows.length}</span> items
 </span>
 )
 }
 actions={
 ...
 <div className="rounded-xl border border-dashed border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-10 text-center shadow-sm dark:border-slate-600 /50">
 <LayoutList className="mx-auto h-10 w-10 text-slate-300 dark:text-[var(--ats-text-muted)]" />
 <div className="mt-2 text-base font-semibold text-[var(--ats-text)]">You&apos;re all caught up</div>
 <div className="mt-1 text-sm text-[var(--ats-text-muted)]">No actionable items right now.</div>
 </div>
 ) : (
 <div className={`${UI.enterprise.elevatedCard} overflow-hidden`}>
 <div className="flex items-center gap-2 border-b border-[var(--ats-border)] px-4 py-3">
 <LayoutList className="h-4 w-4 text-slate-400" />
 <span className="text-xs font-semibold uppercase tracking-wide text-[var(--ats-text-muted)]">
 Work queue
 </span>
 </div>
 ...
 <thead>
 <tr className="sticky top-0 z-10 border-b border-[var(--enterprise-table-border)] bg-[var(--enterprise-table-header)] /98">
 {["Activity","Type","Candidate","Job","Owner","When","Priority"].map((h) => (
 <th
 key={h}
 className="px-5 py-3 text-left text-xs font-semibold uppercase tracking-wide text-[var(--ats-text-muted)]"
 >
 {h}
 </th>
 ))}
 ...
 <td className="px-5 py-3 font-medium text-[var(--ats-text)]">
 {row.link ? (
 ...
 <td className="px-5 py-3 text-[var(--ats-text)]">{row.candidate}</td>
 <td className="px-5 py-3 text-[var(--ats-text)]">{row.job}</td>
 <td className="px-5 py-3 text-[var(--ats-text-muted)]">{row.recruiter}</td>
 <td className="px-5 py-3 text-[var(--ats-text-muted)]">{row.date}</td>
