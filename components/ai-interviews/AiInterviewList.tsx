...
import { BrainCircuit, Plus, RefreshCw } from "lucide-react";
...
export default function AiInterviewList(){
  ...
  async function load(){setLoading(true);setError("");try{const r=await fetch("/api/ai-interviews",{cache:"no-store"});const d=await r.json();if(!r.ok)throw new Error(d.error||"Failed to load");setRows(d.interviews||[]);}catch(e){setError(e instanceof Error?e.message:"Failed to load");}finally{setLoading(false);}}
  useEffect(()=>{void load();},[]);
