...
import { storeDesktopSnapshot } from "@/lib/aiInterviews/storage";
...
export async function POST(request: Request) {
  ...
  if (!interview.screen_share_enabled) return NextResponse.json({ error: "Screen capture is not enabled" }, { status: 409 });
  ...
  try {
    ...
    const snapshot = await storeDesktopSnapshot({ interviewId: Number(interview.id), bytes, mimeType: contentType });
    return NextResponse.json({ captured: true, size: snapshot.size });
  } catch (error) {
    console.error("[ai-interviews] desktop snapshot upload", error);
    return NextResponse.json({ error: error instanceof Error ? error.message : "Desktop snapshot could not be stored" }, { status: 400 });
  }
}
