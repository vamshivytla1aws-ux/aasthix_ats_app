...
export default function AlertsPage() {
 ...
 <p className="text-sm text-[var(--ats-text-muted)]">Saved views and routing rules can live here next.</p>
 </FilterDrawer>
 ...
 <span>
 <span className="font-semibold text-[var(--ats-text)]">{data?.unreadCount ?? 0}</span> unread
 </span>
 )
 }
 actions={
 ...
 toolbar={
 <div className="flex flex-wrap items-center gap-2 rounded-xl border border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-3 shadow-sm /80">
 <input
 value={q}
 onChange={(e) => setQ(e.target.value)}
 placeholder="Search alerts..."
 className="w-full min-w-0 flex-1 rounded-xl border border-gray-200 bg-[var(--ats-bg-panel)] px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 sm:min-w-[220px] dark:border-slate-600 dark:bg-slate-950/50"
 />
 <select
 value={status}
 onChange={(e) => setStatus(e.target.value as typeof status)}
 className="w-full rounded-xl border border-gray-200 bg-[var(--ats-bg-panel)] px-3 py-2 text-sm sm:w-auto dark:border-slate-600 dark:bg-slate-950/50"
 >
 ...
 className="w-full rounded-xl border border-gray-200 bg-[var(--ats-bg-panel)] px-3 py-2 text-xs font-semibold text-[var(--ats-text)] hover:bg-gray-50 disabled:opacity-50 sm:w-auto dark:border-slate-600"
 >
 Mark visible read
 </button>
 </div>
 ...
 <div key={i} className="h-10 animate-pulse rounded bg-[var(--ats-bg-subtle)]" />
 ))}
 </div>
 </div>
 ) : rows.length === 0 ? (
 <div className="rounded-xl border border-dashed border-[var(--ats-border)] bg-[var(--ats-bg-panel)] p-8 text-center shadow-sm dark:border-slate-600 /50">
 <div className="text-lg font-semibold text-[var(--ats-text)]">No alerts found</div>
 <div className="mt-1 text-sm text-[var(--ats-text-muted)]">Try changing filters or check back later.</div>
 </div>
 ) : (
 ...
 <thead className="sticky top-0 z-10 bg-[var(--enterprise-table-header)] /98">
 ...
 <td className="px-4 py-3 break-words text-[var(--ats-text)]">
 <span>{r.message}</span>
 ...
 <td className="px-4 py-3 text-[var(--ats-text-muted)]">{fmt(r.created_at)}</td>
 <td className="px-4 py-3 text-[var(--ats-text-muted)]">{r.expires_at ? fmt(r.expires_at) :"—"}</td>
